...
// this one.
// This shutdown function calls exit(), immediately short-circuiting any other
// shutdown functions,
// such as those registered by the devel.module for statistics.
register_shutdown_function('statusShutdown');

function statusShutdown()
{
    global $t;
    $t['fin'] = microtime(true);
    if (!empty($_GET['timers'])) {
        echo " start({$t['start']})";
        echo " bootstrap({$t['bootstrap']})";
        echo " fin({$t['fin']})";
        echo "\n";
    }
    exit();
...
// Print all errors.
if ($errors) {
    $errors[] = 'Errors on this server will cause it to be removed from the '
        . 'load balancer.';
...
// Exit immediately, note the shutdown function registered
