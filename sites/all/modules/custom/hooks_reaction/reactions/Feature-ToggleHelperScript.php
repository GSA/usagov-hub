...
hooks_reaction_add('HOOK_node_presave',
    function ($node) {
        ...
        if ( $node->type !== 'text_content_type' && $node->type !== 'html_content_type' ) {
            return;
        }
        ...
        if ($nodeNew->title != $nodeOld->title) {
            ...
            if (!empty($refNid)) {
                ...
                $nodeRef->field_gob_feature_toggle_url['und'][0]['value'] = "/novedades/" . $newPath;
            } else {

                $refNid = db_query("SELECT entity_id FROM {field_data_field_usa_feature_toggle_url} WHERE field_usa_feature_toggle_url_value LIKE '/features/" . $oldPath . "'")->fetchField(0);
                if (!empty($refNid)) {
                    // we found referenced node
                    $nodeRef = node_load($refNid);
                    // So lets save a new revision of this $nodeRef
                    ...
                    // While savng it in the needs_review (workflow) state
                    $nodeRef->workbench_moderation_state_new = 'needs_review';
                    $nodeRef->field_usa_feature_toggle_url['und'][0]['value'] = "/features/" . $newPath;
                }
            }
            ...
            if (!empty($nodeRef)) {
                ...
                drupal_set_message(
                    "Note: Since you have updated \"{$node->title}\" asset, referenced toggle field on \"{$nodeRef->title}\" has been updated and a new revision "
...
/*
 * Same function can be found in child site.
 * If you change below function, DON't forgot to update in child-site as well.
 *
 */
