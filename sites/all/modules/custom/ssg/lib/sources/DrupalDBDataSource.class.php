...
class DrupalAPIDataSource extends DataSource
{
  public function getEntities($since=0)
  {
      ...
      $server    = ( !empty($this->ssg->config['drupalAPI']['server']) )    ? $this->ssg->config['drupalAPI']['server']    : 'https://www.usa.loc';
      $url       = ( !empty($this->ssg->config['drupalAPI']['url']) )       ? $this->ssg->config['drupalAPI']['url']       : '/';

      #$siteName = $this->siteName;
      $siteName = ( !empty($this->ssg->config['drupalAPI']['siteName']) ) ? $this->ssg->config['drupalAPI']['siteName'] : 'USA.gov';
