...
function getReport(reqId, noInit) {
            ...
            <div id="reportDownloadingUi">\
                ...
                <div id="reportDownloadingUiText" style="background-color: white; border: 1px solid gray; position: absolute; margin-left: 33%; width: 33%; line-height: 25px; padding: 20px; top: 200px;">\
                    <span id="reportDownloadingUiProgress">\
                        <img style="float: left; padding-right: 10px" src="https://usagov.platform.gsa.gov.dfrey-mbp.net/sites/all/modules/taxonomy_manager/images/ajax-loader.gif" />\
                        Processing, please wait...\
                    </span>\
                    <span id="reportDownloadingUiComplete" style="display: none;">\
                        Report generation complete, you can <a download="site_structure_taxonomy_report.csv">download it now</a>, or <a href="javascript: jQuery(\'#reportDownloadingUi\').remove(); void(0);">close this window</a>.\
                    </span>\
                </div>\
            <div>\
        ';
    ...
    // Request the report
    var ajax = jQuery.get('/content-tag-report/export-site-strucutre-taxonomy?reqid='+reqId, function (data) {
        if ( data == 'working' ) {
            ...
            jQuery('#reportDownloadingUiComplete a').eq(0).attr('href', 'data:text/plain;charset=utf-8,'+encodeURIComponent(data));
